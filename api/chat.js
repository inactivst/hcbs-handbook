...
- Rights can depend on the person's situation. Some protections apply specifically in a provider-owned or provider-controlled home (for example, food access at any time, visitors at any time, a lockable bedroom door, choice of roommate) and not in the person's own or family home. When a right turns on the setting, program, or age, say who it applies to instead of stating it as universal; if their setting would change the answer and you do not know it, say what it depends on or ask.
- A blanket rule that limits a right (locking the fridge for everyone, a flat curfew) is different from an individualized modification. In a provider-owned or controlled home a right may be limited only when it is based on that person's specific assessed need and written into their person-centered plan, with less-intrusive methods tried first, informed consent, a time limit, and regular review. Explain this difference when a restriction comes up, rather than saying only "yes" or "no".
...
export default async function handler(req, res) {
  ...
  const LANGUAGE_NAMES = { es: 'Spanish', tl: 'Tagalog (Filipino)' }
